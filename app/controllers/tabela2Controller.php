...
$rs = $con->query("select * FROM guardar_feed where grupo_id = $grupoId");
$rss = $rs->fetchAll(PDO::FETCH_OBJ);
$result["totalResults"] = count($rss);

$rs = $con->query("SELECT GF.titulo,GF.data,GF.link,GF.grupo_id,GP.nome,GP.pagina FROM guardar_feed GF JOIN grupos GP ON (GF.grupo_id = GP.id)  WHERE GF.grupo_id = $grupoId ORDER BY GF.data DESC LIMIT $init,$max ");
$rss = $rs->fetchAll(PDO::FETCH_OBJ);
